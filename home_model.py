...
def smotenc_split_then_split_data(X, y):
    categorical_features_idx = [
        i for i, feat in enumerate(HOME_CATEGORICAL_FEATURES) 
        if feat in HOME_CATEGORICAL_FEATURES
    ]    
    sm = SMOTENC(
...
def class_imbalance_test():
    ...
    # Helper function
    def add_evals(df: pd.DataFrame, model: AtHomeDiabetesModel, X_test: pd.DataFrame, y_test: pd.Series) -> None: 
    ...
    xgb_params_no_class_weight = {'colsample_bytree': 0.8, 'gamma': 1.5, 'max_depth': 3, 'min_child_weight': 10, 'subsample': 1.0}
    
    # DO NOTHING
    rf_model = AtHomeDiabetesModel(RandomForestClassifier(**rf_params_no_class_weight, random_state=RANDOM_STATE))
    xgb_model = AtHomeDiabetesModel(XGBClassifier(**xgb_params_no_class_weight, random_state=RANDOM_STATE))
    ...
    xgb_params = {'colsample_bytree': 0.8, 'gamma': 1.5, 'max_depth': 3, 'min_child_weight': 10, 'scale_pos_weight': 4, 'subsample': 1.0}
    ...
    print(f"-"*5, "At-Home Model Output", "-"*5)
